...
public:
	...
	public:
		...
		iterator(DArray<T>* _Owner, int _Idx)
			: m_Owner(_Owner)
			, m_Idx(_Idx)
		{
		}
...
template<typename T>
void DArray<T>::push_back(const T& _Data)
{
	// 현재 힙공간에 데이터가 꽉 찬 경우
...
template<typename T>
void DArray<T>::reserve(int _Capacity)
{
	if (0 == _Capacity)
		_Capacity = 1;
...
template<typename T>
void DArray<T>::resize(int _Size)
{
...
template<typename T>
typename DArray<T>::iterator DArray<T>::erase(iterator& _TargetIter)
{
...
template<typename T>
DArray<T>& DArray<T>::operator=(const DArray<T>& _Other)
{
